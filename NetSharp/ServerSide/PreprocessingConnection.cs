...
namespace NetSharp.ServerSide
{
    ...
    class PreprocessingConnection
    {
        ...
        public async Task Start(Socket socket, Func<Connection, ConnectedClient, ConnectingHandler> handlerFactory)
        {
            ...
            try
            {
                ...
                switch (init.CommandCode)
                {
                    case NetSharpProtocol.Commands.NEW_CLIENT:
                        {
                            #region ~Подключение нового клиента.~
                    ...
                    case NetSharpProtocol.Commands.NOT_NEW_CLIENT:
                        {
                            #region ~Подключение от уже существующего клиента.~
